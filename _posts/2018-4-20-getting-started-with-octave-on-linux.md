...
Mostly Octave was build with MatLab comptability in mind. But some Octave only syntax exists. for example, MatLab accepts only single quates (`'...'`) while Octave accepts both single and double quates (`"..."`). For more information on the differences check out [this wiki page](https://en.wikibooks.org/wiki/MATLAB_Programming/Differences_between_Octave_and_MATLAB).
